...
# conn_url = 'postgresql://postgres:pwd4APAN5310@localhost/abc_brokers'
...
```
    ...
    <ipython-input-2-59e6e951b0ac> in <module>
          4 
          5 # Create an engine that connects to PostgreSQL server
    ----> 6 engine = create_engine(conn_url)
          7 
    ...
    ~\Anaconda3\lib\site-packages\sqlalchemy\dialects\postgresql\psycopg2.py in dbapi(cls)
        582     @classmethod
        583     def dbapi(cls):
    --> 584         import psycopg2
        585         return psycopg2
...
CREATE TABLE Ads_Priorities(
	ads_priority_id	 VARCHAR(10),
	ads_priority	VARCHAR(225),
	PRIMARY KEY (ads_priority_id)
);

CREATE TABLE Ads_Priorities_Respondents(
respondent_id VARCHAR (10),
	ads_priority_id	 VARCHAR(10),
            rank INTEGER,
	PRIMARY KEY (respondent_id, ads_priority_id),
             FOREIGN KEY (respondent_id) REFERENCES Respondents_Info,
	FOREIGN KEY (ads_priority_id) REFERENCES Ads_Priorities
);
...
# Delete the original movie_genres columns, no longer necessary
del df['DevType']
...
# Add dev_type_id to the main dataframe
...
```
    ...
    <ipython-input-5-f42ba1f7edb7> in <module>
          1 # load  data to the database
    ----> 2 Ads_Actions.to_sql(name='ads_actions', con=engine, if_exists='append', index=False)
    

    NameError: name 'Ads_Actions' is not defined
...
```
    ...
    <ipython-input-4-1cace9879baa> in <module>
          1 # Map AdActions_id
    ----> 2 AdActions_id_list = [Ads_Actions.ads_action_id[Ads_Actions.ads_action == i].values[0] for i in df29.ads_action]
          3 
          4 # Add AdBlockerReasons_id to the main dataframe
          5 df29.insert(113, 'ads_action_id', AdActions_id_list)
    

    NameError: name 'df29' is not defined
...
```
    ...
    <ipython-input-2-5a309e4397fe> in <module>
          1 # Map RaceEthnicity_id
    ----> 2 RaceEthnicity_id_list = [Race_Ethnicities.race_ethnicity_id[Race_Ethnicities.race_ethnicity == i].values[0] for i in df37.race_ethnicity]
          3 
          4 # Add AdBlockerReasons_id to the main dataframe
          5 df37.insert(109, 'race_ethnicity_id', RaceEthnicity_id_list)
    

    NameError: name 'df37' is not defined
...
```
    ...
    <ipython-input-1-0263d2d0c8b2> in <module>
          1 # load  data to the database
    ----> 2 Race_Ethnicities_Respondents.to_sql(name='race_ethnicities_respondents', con=engine, if_exists='append', index=False)
    

    NameError: name 'Race_Ethnicities_Respondents' is not defined



```python
##################### 40. "Ads_Priorities" Table ###################################
```


```python
# Split Adspriorities and expand row-wise. Store in a temporary dataframe
temp_AdsPriorities_df = df[["respondent_id", "AdsPriorities1", "AdsPriorities2", "AdsPriorities3", "AdsPriorities4", "AdsPriorities5", "AdsPriorities6", "AdsPriorities7"]]
temp_AdsPriorities_df = temp_AdsPriorities_df.set_index("respondent_id").stack().reset_index()
#rename the column
temp_AdsPriorities_df = temp_AdsPriorities_df.rename(columns={"level_1": "AdsPriorities",0:"Rank" })
temp_AdsPriorities_df.head(15)
```
    ...
    <ipython-input-2-ffa666e74e43> in <module>
          1 # Split Adspriorities and expand row-wise. Store in a temporary dataframe
    ----> 2 temp_AdsPriorities_df = df[["respondent_id", "AdsPriorities1", "AdsPriorities2", "AdsPriorities3", "AdsPriorities4", "AdsPriorities5", "AdsPriorities6", "AdsPriorities7"]]
          3 temp_AdsPriorities_df = temp_AdsPriorities_df.set_index("respondent_id").stack().reset_index()
          4 #rename the column
          5 temp_AdsPriorities_df = temp_AdsPriorities_df.rename(columns={"level_1": "AdsPriorities",0:"Rank" })
